...
\makeatletter
\renewcommand{\ICML@appearing}{\textit{DS-GA 1008 Final project report. New York University, 2026.}}
\makeatother
...
\begin{icmlauthorlist}
  ...
  \icmlauthor{Georgios Ioannou (gi2100)}{equal,nyu}
\end{icmlauthorlist}

\icmlaffiliation{nyu}{New York University}
\icmlcorrespondingauthor{Abhinay Krishna Bodi}{ab11958@nyu.edu}
\icmlcorrespondingauthor{Georgios Ioannou}{gi2100@nyu.edu}
...
\caption{Auxiliary diagnostics from supervised and VideoMAE-style masked-video probes, not used for final model selection. Left: masked-video encoders benefit from more temporal context at evaluation time, while supervised saturates earlier. Top right: Fisher separability (inter- over intra-class centroid distance) over the 45 $(\alpha,\zeta)$ combinations on the train split. Bottom right: nearest-centroid validation accuracy on the same 45 combinations. Both confirm that stronger downstream regression coincides with stronger parameter-class separability.}
...
\begin{table*}[t]
\centering
\caption{Main frozen-encoder downstream results and auxiliary diagnostic probes. All MSE values are normalized by train-label statistics and evaluated on the held-out test split after validation-based selection. Multi-seed entries are reported as mean$\pm$std. Auxiliary rows are listed for completeness; the supervised rows used $(\alpha,\zeta)$ labels during pretraining (label-aware diagnostic, not a frozen-SSL submission), and the masked-video rows underperform CNext. Lower is better; bold = best per column.}
\label{tab:results}
\resizebox{\textwidth}{!}{%
...
\midrule
\multicolumn{6}{l}{\emph{Eligible frozen-encoder runs}}\\
...
JEPA + VICReg ($224{\times}224$) & Linear & 0.2485 & 0.1183 & 0.3786 & original baseline \\
JEPA + VICReg ($224{\times}224$) & kNN & 0.2709 & 0.0472 & 0.4946 & original baseline \\
JEPA + VICReg ($96{\times}96$) & Linear & 0.4260 & 0.1129 & 0.7391 & resolution-matched rerun \\
JEPA + VICReg ($96{\times}96$) & kNN & 0.3865 & 0.2374 & 0.5356 & resolution-matched rerun \\
ConvNeXt JEPA + SigReg & Linear & 0.5829 & 0.1872 & 0.9787 & SigReg frozen; export mismatch \\
ConvNeXt JEPA + SigReg & kNN & 0.5924 & 0.2058 & 0.9790 & SigReg frozen; export mismatch \\
\midrule
\multicolumn{6}{l}{\emph{Auxiliary diagnostics (not final submissions)}}\\
Supervised ConvNeXt ($3$ seeds) & Linear & $0.106{\pm}0.006$ & $0.029{\pm}0.006$ & $0.182{\pm}0.017$ & label-supervised reference \\
Supervised ConvNeXt ($3$ seeds) & kNN & $0.106{\pm}0.020$ & $0.022{\pm}0.012$ & $0.190{\pm}0.030$ & label-supervised reference \\
VideoMAE-style mask $0.60$ ($3$ seeds) & Linear & $0.317{\pm}0.024$ & $0.124{\pm}0.014$ & $0.510{\pm}0.040$ & masked-video diagnostic \\
VideoMAE-style mask $0.60$ ($3$ seeds) & kNN & $0.562{\pm}0.035$ & $0.156{\pm}0.020$ & $0.967{\pm}0.054$ & masked-video diagnostic \\
VideoMAE-style mask $0.60$, $50$ ep & Linear & $0.338$ & $0.179$ & $0.497$ & recon-loss decoupling test \\
VideoMAE-style mask $0.60$, $50$ ep & kNN & $0.389$ & $0.194$ & $0.583$ & recon-loss decoupling test \\
VideoMAE-style mask $0.75$ & Linear & $0.299$ & $0.075$ & $0.522$ & mask-ratio sweep \\
VideoMAE-style mask $0.75$ & kNN & $0.672$ & $0.297$ & $1.046$ & mask-ratio sweep \\
VideoMAE-style mask $0.90$ ($2$ seeds) & Linear & $0.255{\pm}0.002$ & $0.108{\pm}0.003$ & $0.402{\pm}0.007$ & mask-ratio sweep \\
VideoMAE-style mask $0.90$ ($2$ seeds) & kNN & $0.503{\pm}0.039$ & $0.217{\pm}0.017$ & $0.789{\pm}0.060$ & mask-ratio sweep \\
\bottomrule
\end{tabular}%
}
\end{table*}
